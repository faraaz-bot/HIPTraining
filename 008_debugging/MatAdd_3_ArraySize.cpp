...
int main()
{
    ...
    float* devA = nullptr;
    assert(hipMalloc(&devA, SIZE) == hipSuccess);
    assert(hipMemcpy(devA, hostA.data(), SIZE, hipMemcpyHostToDevice) == hipSuccess);

    float* devB = nullptr;
    assert(hipMalloc(&devB, SIZE) == hipSuccess);
    assert(hipMemcpy(devB, hostB.data(), SIZE, hipMemcpyHostToDevice) == hipSuccess);

    float* devC = nullptr;
    assert(hipMalloc(&devC, SIZE) == hipSuccess);
    ...
    assert(hipMemcpy(testC.data(), devC, SIZE, hipMemcpyDeviceToHost) == hipSuccess);
